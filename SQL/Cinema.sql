...
ALTER TABLE seat
    DROP CONSTRAINT IF EXISTS uq_seat_screen_row_num;
DROP INDEX IF EXISTS uq_seat_screen_row_num;
ALTER TABLE seat
    DROP CONSTRAINT IF EXISTS uq_seat_hall_row_num;
DROP INDEX IF EXISTS uq_seat_hall_row_num;
-- Tilføj ny constraint
ALTER TABLE seat
    ADD CONSTRAINT uq_seat_hall_row_num UNIQUE (hall_id, row_letter, seat_number);
...
SET max_row_letter  = sub.max_row_letter,
...
BEGIN
    UPDATE seat_layout
    SET max_row_letter  = (SELECT MAX(row_letter)
                           FROM layout_seat
                           WHERE layout_id = NEW.layout_id),
        max_seat_number = (SELECT MAX(seat_number)
                           FROM layout_seat
                           WHERE layout_id = NEW.layout_id)
    WHERE layout_id = NEW.layout_id;
...
INSERT
INTO layout_seat(layout_id, row_letter, seat_number)
SELECT s.layout_id, CHR(ASCII('A') + r)::char(1), n
FROM small s,
     GENERATE_SERIES(0, 3) AS r,
...
WITH h1 AS (
    INSERT INTO hall (hall_number, capacity, layout_id)
        VALUES (1, 40,
                (SELECT layout_id FROM seat_layout WHERE name = 'SmallHall'))
        RETURNING hall_id, layout_id),
     h2 AS (
         INSERT INTO hall (hall_number, capacity, layout_id)
             VALUES (2, 96,
                     (SELECT layout_id FROM seat_layout WHERE name = 'BigHall'))
             RETURNING hall_id, layout_id)
INSERT
INTO seat(hall_id, row_letter, seat_number)
SELECT h1.hall_id, ls.row_letter, ls.seat_number
FROM layout_seat ls
         CROSS JOIN h1
WHERE ls.layout_id = h1.layout_id;

WITH h2ref AS (SELECT hall_id, layout_id FROM hall WHERE hall_number = 2)
INSERT
INTO seat(hall_id, row_letter, seat_number)
...
VALUES ('The Matrix', 136, 'Sci-Fi', '1999-03-31',
       ...
       ('Spirited Away', 125, 'Animation', '2001-07-20',
        'Spirited Away is a Hayao Miyazaki masterpiece about 10-year-old Chihiro, who gets trapped in a magical world of gods and spirits after her parents are turned into pigs; she must work at a bathhouse run by the witch Yubaba to survive, find her courage, reclaim her name, and save her family, encountering strange creatures like No-Face and learning lessons about greed, identity, and compassion.',
        'https://m.media-amazon.com/images/M/MV5BNTEyNmEwOWUtYzkyOC00ZTQ4LTllZmUtMjk0Y2YwOGUzYjRiXkEyXkFqcGc@._V1_.jpg',
        'https://www.fathomentertainment.com/wp-content/uploads/GF23_FathomBanners_2023-02-22_SpiritedAway_1920x700.jpg.jpg'),
       ('Drive', 100, 'Action', '2011-09-15',
        'A mysterious Hollywood action film stuntman gets in trouble with gangsters when he tries to help his neighbor''s husband rob a pawn shop while serving as his getaway driver.',
        'https://m.media-amazon.com/images/M/MV5BYTFmNTFlOTAtNzEyNi00MWU2LTg3MGEtYjA2NWY3MDliNjlkXkEyXkFqcGc@._V1_.jpg',
        'https://m.media-amazon.com/images/M/MV5BNTM1NDk2OTE3M15BMl5BanBnXkFtZTcwMTM1NTgxNQ@@._V1_.jpg'),
       ('The Godfather', 175, 'Drama', '1972-03-14',
...
INSERT INTO Screening(movie_id, hall_id, screening_date, start_time,
                      available_seats)
VALUES ((SELECT movie_id FROM movie WHERE title = 'The Matrix'),
        (SELECT hall_id FROM hall WHERE hall_number = 1),
        date '2026-02-10',
        time '19:30:00',
        (SELECT COUNT(*)
         FROM seat
         WHERE hall_id = (SELECT hall_id FROM hall WHERE hall_number = 1))),
       ...
       ((SELECT movie_id FROM movie WHERE title = 'Spirited Away'),
        ...
        (SELECT COUNT(*)
         FROM seat
         WHERE hall_id = (SELECT hall_id FROM hall WHERE hall_number = 2))),
       ((SELECT movie_id FROM movie WHERE title = 'Spirited Away'),
        (SELECT hall_id FROM hall WHERE hall_number = 1),
        date '2026-03-12',
        time '17:30:00',
        (SELECT COUNT(*)
         FROM seat
         WHERE hall_id = (SELECT hall_id FROM hall WHERE hall_number = 1))),
       ((SELECT movie_id FROM movie WHERE title = 'Drive'),
        (SELECT hall_id FROM hall WHERE hall_number = 1),
        date '2026-01-27',
        time '20:00:00',
        (SELECT COUNT(*)
         FROM seat
         WHERE hall_id = (SELECT hall_id FROM hall WHERE hall_number = 2))),
       ((SELECT movie_id FROM movie WHERE title = 'The Godfather'),
        (SELECT hall_id FROM hall WHERE hall_number = 2),
        date '2026-02-16',
        time '18:30:00',
        (SELECT COUNT(*)
